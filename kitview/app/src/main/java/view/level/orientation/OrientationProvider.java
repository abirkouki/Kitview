package view.level.orientation;
...
public final class OrientationProvider implements SensorEventListener {
	...
	private static final int MIN_VALUES = 10;//300;//10;//20;
    ...
    private OrientationProvider(Activity context) {
    	_context = context;
		//this.displayOrientation = Level.getContext().getWindowManager().getDefaultDisplay().getRotation();
    	this.displayOrientation = _context.getWindowManager().getDefaultDisplay().getRotation();
		
	}

    public static OrientationProvider getInstance(Activity context) {
        if (provider == null) {
     ...
     */
    public boolean isSupported(Activity context) {
        if (supported == null) {
            	if(context != null){
     ...
     */
    public void startListening(OrientationListener orientationListener,Activity context) {
    	// load calibration
    	calibrating = false;
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if (calibrating) {
			...
			Editor editor = ScenariosActivity.getContext().getPreferences(Context.MODE_PRIVATE).edit();
	...
	public final void resetCalibration() {
		...
		try {
			success = ScenariosActivity.getContext().getPreferences(Context.MODE_PRIVATE).edit().clear().commit();
